...
export default async function boltAnalysis(userData) {
  const [bolts, properties] = await Promise.all([
    getAllBolts(),
    getAllPropertyClasses(),
  ]);

  if (!bolts) console.log('[runAnalysis] Failed to fetch all bolts');
  if (!properties) console.log('[runAnalysis] Failed to fetch all properties');
  ...
  try {
    ...
    for (const curBolt of bolts) {
      ...
      //       continue;
      //     }
      for (const curBoltProperty of properties) {
